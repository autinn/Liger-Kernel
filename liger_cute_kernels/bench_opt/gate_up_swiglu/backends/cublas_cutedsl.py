"""PR #1277 CuTeDSL SwiGLU with two expert-batched cuBLAS GEMMs.

This provider pins the implementation published by Liger-Kernel PR #1277:
``2199094898dbfef2bc75c1d8b6566baf2ff61695``.
https://github.com/linkedin/Liger-Kernel/blob/2199094898dbfef2bc75c1d8b6566baf2ff61695/src/liger_kernel/ops/cutedsl/ops/swiglu.py
...
``860f9c79725f84cfdf7b12d913cbfbd002f8a4347e0cc06447e94116db70c2aa``.
"""
...
def ensure_contiguous(function):
    @functools.wraps(function)
    def wrapper(ctx, *args, **kwargs):
        contiguous_args = [arg.contiguous() if isinstance(arg, torch.Tensor) else arg for arg in args]
        contiguous_kwargs = {
            key: value.contiguous() if isinstance(value, torch.Tensor) else value for key, value in kwargs.items()
        }
        return function(ctx, *contiguous_args, **contiguous_kwargs)

    return wrapper


def infer_device_arch(device_id=0):
...
# Static probe: are the Blackwell packed-f32x2 SFU symbols importable? They exist
# in the CUTLASS wheel on every arch, so presence does NOT imply usability -- the
# actual compute capability is checked per launch (see ``_is_blackwell``) from the
# input tensor's device, not frozen once at import.
try:
...
# Static probe: is the optional ``apache-tvm-ffi`` package importable? The fast
# path passes PyTorch tensors straight through TVM-FFI (no ``from_dlpack``). This
# is a calling convention and is independent of the GPU arch.
try:
...
def _is_blackwell(device=None) -> bool:
    """Whether ``device`` is a **data-center Blackwell** (sm_100 / sm_103) that
    supports the packed-f32x2 SFU intrinsics used by the fast path.

    Gated to the sm_10x data-center parts only — **B200 (sm_100)** and
    **B300 (sm_103)**. Hopper (**H100/H200**, sm_90) and *consumer* Blackwell
    (RTX 50xx, sm_120) do NOT get the packed path: the packed-f32x2 SFU ops are
    a data-center-Blackwell feature (their Python symbols exist on every arch,
    so presence != usability), and those parts run the correct scalar/vectorized
    fallback instead. Checked per launch from the *input tensor's* device: a
    process may hold tensors on GPUs of different archs, and
    ``torch.cuda.set_device`` can move the current device after import.
    """
    try:
...
def _dispatch_plan(numel: int, vec: int, tile: int, device=None, aligned: bool = True):
    ...
    ``device`` is the input tensor's device; the Blackwell capability is probed
    from it per launch so mixed-arch / post-``set_device`` processes route each
    tensor by its own GPU. ``aligned`` must be True for the fast path: its 128-bit
    copies assume 16-byte-aligned base pointers, which a contiguous view with a
    non-16-byte ``storage_offset`` (e.g. ``base[1:]``) does not guarantee.
    """
    blackwell = _PACKED_OPS_PRESENT and _is_blackwell(device)
    use_fast = blackwell and _TVM_FFI_PRESENT and aligned and vec >= 2 and (numel % tile) == 0
    predicated = (numel % tile) != 0
    packed_math = blackwell and vec >= 2
    return use_fast, predicated, packed_math


def _is_16b_aligned(*tensors: torch.Tensor) -> bool:
    """Whether every tensor's base pointer is 16-byte aligned.

    The vectorized fast path emits 128-bit (16-byte) loads/stores, which fault or
    corrupt on a misaligned base. ``@ensure_contiguous`` does not reallocate an
    already-contiguous input, so a non-zero ``storage_offset`` can leave the base
    pointer unaligned even when ``numel`` is tile-aligned.
    """
    return all(t.data_ptr() % 16 == 0 for t in tensors)
...
def prepare(x, gate_weight, up_weight, expert_ids):
    ...
    if tokens % experts != 0:
        raise ValueError("cuBLAS comparison expects balanced blocked routing")
    rows_per_expert = tokens // experts
    return {
        "x": x.view(experts, rows_per_expert, hidden),
        "gate_weight": gate_weight.transpose(1, 2),
        "up_weight": up_weight.transpose(1, 2),
        "gate": torch.empty(
            experts,
            rows_per_expert,
            intermediate,
...
def launch(state):
    ...
    return swiglu_forward(state["gate"], state["up"])[2].view(
        -1,
        state["gate"].shape[-1],
    )
